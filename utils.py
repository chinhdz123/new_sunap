...
def find_circles_in_small_img(img,w_s, w_en):
    ...
    classes = output["instances"].pred_classes if output["instances"].has("pred_classes") else None
    if boxes is not None:
        boxes = convert_boxes(boxes)
    circles1= []
    circles2= []
    circles3= []
    for box,classs in zip(boxes,classes):
        if classs == 1:
            x0, y0, x1, y1 = box
            ...
            if center_X-r > w_s and center_X+r < w_en: 
                circles1.append([center_X,center_Y,r])
        elif classs == 2:
            x0, y0, x1, y1 = box
            center_X =  int((x1+x0)/2)
            center_Y = int((y1+y0)/2)
            r = int((x1-x0)/2)
            if center_X-r > w_s and center_X+r < w_en: 
                circles2.append([center_X,center_Y,r])
        elif classs == 3:
            x0, y0, x1, y1 = box
            center_X =  int((x1+x0)/2)
            center_Y = int((y1+y0)/2)
            r = int((x1-x0)/2)
            if center_X-r > w_s and center_X+r < w_en: 
                circles3.append([center_X,center_Y,r])
